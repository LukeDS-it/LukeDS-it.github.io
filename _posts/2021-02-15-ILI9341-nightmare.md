...
It features a 240x320 lcd display with LED backlight, a touch screen and a SD card reader (note:
...
- Neither the display or the SD card work (white screen)
...
And then came failure: after a call to the SD initialization the TFT became unresponsive.
...
And think by yourself. Since I noticed that the display still sometimes flickered with the color I
